...
import Avatar from "./GuildAvatar"
...
export default function ServersList({ servers }: ServersListProps): JSX.Element {
    return (
        <Wrapper>
            <CircleButton name={"Home"} icon={DiscordLogo} color={brandColor} iconColor="#dcddde" />
            <GuildSperator />
            {servers.map((server) => {
                return (
                    <Avatar {...server} />
            ...
            <GuildSperator />
            <CircleButton name={"There's a super gift waiting for you!"} icon={LightningBoltIcon} color={success} />
        </Wrapper>
    )
}